# Housing model with a Life cycle 
# Yeongwoong Do (2024-04-26)

using LinearAlgebra, Interpolations, Plots, Printf, Dates, MAT, Optim, SparseArrays, NLopt, Roots

###############################################################
###################### (subfunctions) #########################
###############################################################

# income tax 
function income_tax(y)
    # income tax rate
    t_y     = [0.06, 0.15, 0.24, 0.35, 0.38]
    thres_y = [0.68, 1.89, 3.09, 4.99, 9.59]
    d_y     = [0.3, 0.6, 0.85, 0.95, 0.98]
    d_thres = [0.15, 0.45, 1.35, 3.00]
    tax     = [0, 0.0252, 0.1875, 0.4615, 1.1136]
    if y <= thres_y[1]
        if y<= d_thres[1]
            return (y*d_y[1])*t_y[1]
        elseif y<= d_thres[2]
            return (d_thres[1]*d_y[1] + (y-d_thres[1])*d_y[2])*t_y[1]
        else
            return (d_thres[1]*d_y[1] + (d_thres[2]-d_thres[1])*d_y[2]+(y-d_thres[2])*d_y[3])*t_y[1]
        end
    elseif y <= thres_y[2]
        if y<= d_thres[3]
            return tax[2] + ((y-thres_y[1])*d_y[3])*t_y[2]
        else
            return tax[2] + ((d_thres[3]-thres_y[1])*d_y[3]+(y-d_thres[3])*d_y[4])*t_y[2]
        end
    elseif y <= thres_y[3]
        if y<= d_thres[4]
            return tax[3] + ((y-thres_y[2])*d_y[4])*t_y[3]
        else
            return tax[3] + ((d_thres[4]-thres_y[2])*d_y[4]+(y-d_thres[4])*d_y[5])*t_y[3]
        end
    elseif y <= thres_y[4]
        return tax[4] + (y-thres_y[3])*d_y[5]*t_y[4]
    else
        return tax[5] + (y-thres_y[4])*d_y[5]*t_y[5]
    end
end;

# housing tax
function housing_tax_cost(gamma, delta, h, h_prime, k_adj, cutoff, q, tau_p, HRET)
    transaction_cost = gamma*(q*h_prime + q*(1-delta)*h)*(h_prime!=h)
    property_tax = tau_p*q*h_prime
    HRET_tax = HRET*k_adj*(q*h_prime - cutoff)*(q*h_prime >= cutoff)
    return transaction_cost + property_tax + HRET_tax
end

# RHS of Renter's problem
function RHS_renter(a_prime, bar_x, sigma, alpha, beta, V_itp, Pi, s_prob, rho, z_idx, t, b_bar) 
    x = bar_x - a_prime
    if x <0
        return -1e4
    end
    u = (x^(1-sigma)/(1-sigma))*(1/rho)^((1-alpha)*(1-sigma))
    T = length(s_prob)
    if t==T   
        return u + b_bar*(a_prime)^(1-sigma)/(1-sigma)
    else
        exp = 0
        K0, K = size(Pi)
        @inbounds for k=1:K
            # 딕셔너리에서 보간 함수를 가져옴
            itp = V_itp["func1_$(k)"]
            exp = exp + Pi[z_idx,k]*itp(a_prime)
        end 
        return u + beta*s_prob[t]*exp
    end
end;

# RHS of onwer-occupier's problem
function RHS_owner(a_prime, h_prime, h_prime_idx, bar_x, sigma, alpha, beta, V_itp, Pi, s_prob, q, z_idx, t, h_cost, b_bar) 
    c = bar_x -a_prime - q*h_prime - h_cost
    if c <=0 || h_prime<=0
        return -1e4
    end
    T = length(s_prob)
    u = ((c/alpha)^alpha * (h_prime/(1-alpha))^(1-alpha))^(1-sigma)/(1-sigma)
    if t==T
        return u + b_bar*(a_prime+q*h_prime)^(1-sigma)/(1-sigma)
    else
        exp = 0; K0, K = size(Pi);
        @inbounds for k=1:K
            itp = V_itp["func$(h_prime_idx)_$(k)"]
            exp = exp + Pi[z_idx,k]*itp(a_prime)
        end       
        return u + beta*s_prob[t]*exp
    end
end;

# RHS of landlord's problem
function RHS_landlord(a_prime, h_prime, h_prime_idx, bar_x, sigma, alpha, beta, phi, V_itp, Pi, s_prob, rho, q, z_idx, t, tau_R, h_cost, b_bar)
    x = bar_x -a_prime - (q-rho*(1-tau_R))*h_prime - h_cost - phi
    if x <0
        return -1e4
    end
    T = length(s_prob)
    u = (x^(1-sigma)/(1-sigma))*(1/(rho*(1-tau_R)))^((1-alpha)*(1-sigma))
    if t==T
        return u + b_bar*(a_prime+q*h_prime)^(1-sigma)/(1-sigma)
    else
        exp = 0; K0, K = size(Pi);
        @inbounds for k=1:K
            itp = V_itp["func$(h_prime_idx)_$(k)"]
            exp = exp + Pi[z_idx,k]*itp(a_prime)
        end       
        return u + beta*s_prob[t]*exp
    end
end;

# aggregation을 위한 transtion matrix 생성 함수
function BTM(pol_a, pol_h, Pi, N,J,K, a_grid, h_grid)
    # 1단계 a, h의 transtion block 만들기
    block_transition = zeros(N*J,N*J,K)
    for k=1:K
        for n=1:N
            for j=1:J
            
                # 값 알아보기 쉽게하기
                h_prime = pol_h[n,j,k]
                a_prime = pol_a[n,j,k]
            
                # h->h' 위치찾기
                h_idx = argmin(abs.(h_grid .- h_prime))                
                # a->a' 위치찾기
                a_idx = argmin(abs.(a_grid .- a_prime))           
                row_num = (j-1)*N + n
                col_num = (h_idx-1)*N + a_idx
                
                if a_prime == a_grid[a_idx] # a'이 a 그리드값과 동일
                    block_transition[row_num, col_num, k] = 1
                
                elseif a_prime > a_grid[a_idx] # a그리드랑 가까운 쪽이 왼쪽인 경우
                    ratio = (a_prime - a_grid[a_idx]) / (a_grid[a_idx+1] - a_grid[a_idx])
                    block_transition[row_num, col_num, k] = 1 - ratio
                    block_transition[row_num, col_num+1, k] = ratio
                
                elseif a_prime < a_grid[a_idx] # a그리드랑 가까운 쪽이 오른쪽인 경우
                    ratio = (a_grid[a_idx] - a_prime) / (a_grid[a_idx] - a_grid[a_idx-1])
                    block_transition[row_num, col_num, k] = 1- ratio
                    block_transition[row_num, col_num-1, k] = ratio
                end
            
            end
        end    
    end
    
    
    # 2단계 block matrix를 배치하기
    W = zeros(N*J*K,N*J*K);

    for k1=1:K
        for k2=1:K
            # block indexing
            r_start = N*J*(k1-1)+1
            r_end   = N*J*k1
            c_start = N*J*(k2-1)+1
            c_end   = N*J*k2
            W[r_start:r_end, c_start:c_end] = Pi[k1,k2].*block_transition[:,:,k1];
        end
    end
    
    return W
end;

#############################################################
####################(Main)###################################
#############################################################

function Housing_PE_LC(q, h_min, rho, phi, cutoff, b_bar, Life_mat, Idio_mat)
    @printf("rho: %.8f\n", rho)
    # save vector
    w_age  = Life_mat[:,1]
    T = length(w_age)
    s_prob = Life_mat[:,2]
    mu     = Life_mat[:,3]
    z_grid = Idio_mat[:,1]
    K = length(z_grid)
    Pi     = Idio_mat[:,2:end]

    # parameters
    sigma   = 2.000;       # inverse of EIS
    alpha   = 0.778;       # share of non-housing consumption
    r       = 0.024;       # interest rate
    kappa   = 0.013;       # spread of mortgage debt
    beta    = 0.975;       # time discount factor
    delta   = 0.030;       # depreciation ratio of housing asset
    LTV       = 0.600;       # LTV limit
    gamma   = 0.010;       # transaction cost
    tau_p   = 0.001;       # property tax rate (effective rate)
    tau_R   = 0.010;       # rental income tax rate (effective rate)
    HRET    = 0.010;       # HRET tax rate (nominal rate)
    k_adj   = 0.60;        # adjustment coefficient 
    pen     = 0.109;       # pension 
    R       = 61-19;       # retire age
    DTI     = 0.50;        # DTI limit
    n       = 0;           # net population growth rate 

    # A grid, H_grid
    a_min = -10
    a_max = 20
    N = 50
    h_max = 20
    J = 10
    # a_grid 생성
    a_debt_grid = -(LinRange(sqrt(-a_min), sqrt(0), Int(N/2))).^2
    delta = a_debt_grid[end] - a_debt_grid[end-1]
    a_saving_grid = exp.(LinRange(log(delta), log(a_max), Int(N/2)))
    a_grid = [a_debt_grid; a_saving_grid]
    a_lb_renter = 0
    # h_grid 생성
    h_grid = exp.(LinRange(log(h_min), log(h_max), J-1))
    h_grid = [0; h_grid];

    # 빈공간 만들기
    V = zeros(N,J,K,T);
    pol_stance = zeros(N,J,K,T);
    pol_a = zeros(N,J,K,T);
    pol_h = zeros(N,J,K,T);
    pol_s = zeros(N,J,K,T);
    V_itp = Dict()
    Threads.@threads for k=1:K
        Threads.@threads for j=1:J
            interp = LinearInterpolation(a_grid, V[:,j,k,end])
            V_itp["func$(j)_$(k)"] = interp
        end
    end

@time begin
    
    for t=T:-1:1  # Backward!  
            
            Threads.@threads for z_idx=1:K
                Threads.@threads for h_idx=1:J
                    Threads.@threads for a_idx=1:N
                        
                        z = z_grid[z_idx]
                        h = h_grid[h_idx]
                        a = a_grid[a_idx]
                        y = w_age[t]*z
                        tax_base = y + r*a*(a>0)

                        ###########################################
                        # Renter's problem W(.,h'=0)
                        bar_x = y -income_tax(tax_base) + pen*(t>=R) + (1+r)*a + q*(1-delta)*h*(1-gamma) + kappa*a*(a<0)  # maximum expenditure
                        a_ub_renter = min(bar_x, a_max)
                        if a_ub_renter < a_lb_renter # 말도 안되는 상황이 오면
                            W_R = -1e4
                            W_R_pol_a = 0
                            W_R_pol_s = 0 
                        else
                            function f_rent(x)
                                return -RHS_renter(x, bar_x, sigma, alpha, beta, V_itp, Pi, s_prob, rho, z_idx, t, b_bar) 
                            end
                            results = Optim.optimize(f_rent, a_lb_renter, a_ub_renter)
                            W_R = -Optim.minimum(results)
                            W_R_pol_a = Optim.minimizer(results)
                            W_R_pol_s = (1-alpha)*(bar_x- W_R_pol_a)/rho
                        end
                        ################################################
                        
                        ############################################
                        # homeowner's problem W(.,h'>0)
                        W_temp = zeros(J-1,2);
                        pol_a_temp = zeros(J-1,2);
                        pol_stance_temp = zeros(J-1,2);
                        bar_x = y -income_tax(tax_base) + pen*(t>=R) +(1+r)*a+q*(1-delta)*h+kappa*a*(a<0) # maximum expenditure
            
                        Threads.@threads for h_prime_idx = 2:J
                            h_prime = h_grid[h_prime_idx]
                            h_cost = housing_tax_cost(gamma, delta, h, h_prime, k_adj, cutoff, q, tau_p, HRET)
                            if t == T
                                a_lb_owner = 0  # t==T일 경우, 0을 사용
                            else
                                a_lb_owner = max(-LTV*q*h_prime, -DTI*y/(0.1+r+kappa), a_min)
                            end
                            a_ub_owner = min(bar_x - q*h_prime - h_cost, a_max)
                            a_ub_landlord = min(bar_x - q*h_prime - h_cost - phi , a_max)

                            # Owner-occupier's  problem
                            if a_ub_owner < a_lb_owner # 말도 안되는 상황이 오면
                                W_temp[h_prime_idx-1,1]=-1e4
                                pol_a_temp[h_prime_idx-1,1]= 0
                                pol_stance_temp[h_prime_idx-1,1] = 0
                            else
                                function f_owner(x)
                                    return - RHS_owner(x, h_prime, h_prime_idx, bar_x, sigma, alpha, beta, V_itp, Pi, s_prob, q, z_idx, t, h_cost, b_bar)
                                end
                                results_owner_occupied = Optim.optimize(f_owner, a_lb_owner, a_ub_owner)
                                W_temp[h_prime_idx-1,1] = -Optim.minimum(results_owner_occupied)
                                pol_a_temp[h_prime_idx-1,1] = Optim.minimizer(results_owner_occupied)
                                pol_stance_temp[h_prime_idx-1,1] = 1
                            end

                            # Landlord's problem
                            if a_ub_landlord < a_lb_owner
                                W_temp[h_prime_idx-1,2]=-1e4
                                pol_a_temp[h_prime_idx-1,2]= 0
                                pol_stance_temp[h_prime_idx-1,2] = 0
                            else
                                function f_landlord(x)
                                    return - RHS_landlord(x, h_prime, h_prime_idx, bar_x, sigma, alpha, beta, phi, V_itp, Pi, s_prob, rho, q, z_idx, t, tau_R, h_cost, b_bar) 
                                end
                                results_landlord = Optim.optimize(f_landlord, a_lb_owner, a_ub_landlord)
                                W_temp[h_prime_idx-1,2] = -Optim.minimum(results_landlord)
                                pol_a_temp[h_prime_idx-1,2] = Optim.minimizer(results_landlord)
                                pol_stance_temp[h_prime_idx-1,2] = 2
                            end                           

                        end       
                        # h'>0의 case비교하여서 최적을 선택
                        W_o, idx_W_o = findmax(W_temp)
                        ci = CartesianIndices(W_temp)[idx_W_o]
                        W_o_pol_a = pol_a_temp[ci]
                        W_o_pol_h = h_grid[ci[1]+1]            
                        W_o_stance = pol_stance_temp[ci]
                        if W_o_stance==2
                            W_o_pol_s = (1-alpha)*(bar_x - q*W_o_pol_h -gamma*(q*W_o_pol_h + q*(1-delta)*h)*(W_o_pol_h!= h) -phi)/rho
                        else
                            W_o_pol_s = W_o_pol_h
                        end
                        ###################################        
                        
                        if W_R >= W_o # HH will be a renter
                            V[a_idx, h_idx, z_idx, t]=W_R
                            pol_stance[a_idx, h_idx, z_idx, t]=0
                            pol_a[a_idx, h_idx, z_idx, t] = W_R_pol_a
                            pol_h[a_idx, h_idx, z_idx, t] = 0
                            pol_s[a_idx, h_idx, z_idx, t] = W_R_pol_s
                        else         # HH will be a homeowner
                            V[a_idx, h_idx, z_idx, t]=W_o
                            pol_stance[a_idx, h_idx, z_idx, t]=W_o_stance
                            pol_a[a_idx, h_idx, z_idx, t] = W_o_pol_a
                            pol_h[a_idx, h_idx, z_idx, t] = W_o_pol_h
                            pol_s[a_idx, h_idx, z_idx, t] = W_o_pol_s
                        end
                    end
                end
            end
            
            # 다음 연령에서 사용할 interpolation 만들기
            V_itp = Dict()
            Threads.@threads for k=1:K
                Threads.@threads for j=1:J
                interp = LinearInterpolation(a_grid, V[:,j,k,t])
                V_itp["func$(j)_$(k)"] = interp
                end
            end
            
        end
        
        
        # 시간측정 end
    end


    ###################(Step 2 : Stationary distribution) #################
    # transition matrix
    @time begin
        NJK = N*J*K
        W = spzeros(NJK*T, NJK*T)
    
        # 각 블록을 적절한 위치에 배치
        Threads.@threads for t in 1:T
            rows = ((t-1)*NJK+1):(t*NJK)
            # 죽음
            W[rows, 1:NJK] = BTM(pol_a[:,:,:,t], pol_h[:,:,:,t], Pi, N,J,K, a_grid, h_grid).*(1-s_prob[t]/(1+n))  
            # 생존
            if t<T
                W[rows, (t*NJK+1):(t*NJK+NJK)] = BTM(pol_a[:,:,:,t], pol_h[:,:,:,t], Pi, N,J,K, a_grid, h_grid).*s_prob[t]/(1+n)   
            end
        end
    
        a_0_idx = argmin(abs.(a_grid.- 0));
    
        # psi 찾기
        psi_ini = ones(1, NJK*T)
        
        max_iter = 5000
        tol2     = 1e-6
        stat_iter = 0
        psi_old = psi_ini
        diff_dist = 10
        
        for i=1:max_iter
            psi_new = psi_old*W
            diff_dist = maximum(abs.(psi_new.-psi_old))
            stat_iter +=1
            psi_old = psi_new
            if diff_dist < tol2
                break
            end
        end    

        println("반복횟수: ", stat_iter, "오차: ", diff_dist)
    end
    
        # Aggregation
        psi = psi_old./sum(psi_old)
        psi_mat = reshape(psi,N,J,K,T)
        share_R = sum((pol_stance.== 0) .* psi_mat) 
        share_L = sum((pol_stance.== 2) .* psi_mat)
        share_HRET = sum((q.*pol_h.>cutoff).*psi_mat)
        demand_rental = sum((pol_stance.== 0).*pol_s.* psi_mat) 
        supply_rental = sum((pol_stance.== 2).*(pol_h-pol_s).* psi_mat)
        excess_demand_rental = demand_rental - supply_rental 

        @printf("Renter   share                : %.4f\n", share_R)
        @printf("Landlord share                : %.4f\n", share_L)
        @printf("HRET     share                : %.4f\n", share_HRET)
        @printf("excess demand in rental market: %.4f\n", excess_demand_rental)

        return share_R, share_L, share_HRET, excess_demand_rental, V, pol_a, pol_h, pol_s, psi
end

#############################################################
#############################################################
#############################################################

# Load the micro data
file = matopen("C:/Users/82106/Documents/4. 2024학년도/HRET/calibration/Life_mat.mat")
Life_mat = read(file, "Life_mat")
close(file)
file = matopen("C:/Users/82106/Documents/4. 2024학년도/HRET/calibration/Idio_mat.mat")
Idio_mat = read(file, "Idio_mat")
close(file)

println("------Housing model 추정을 시작합니다.-----------")
start_str = Dates.format(now(), "yyyy-mm-dd HH:MM:SS")
println("시작시각: ", start_str)

function GE_obj(x)
    # 여타 내생변수 값 설정
    q = 3
    h_min = 2.1
    phi = 0.15
    cutoff = 40
    b_bar = 0.50
    s_R, s_L, s_HRET, ex, V, pol_a, pol_h, pol_s, psi = Housing_PE_LC(q, h_min, x, phi, cutoff, b_bar, Life_mat, Idio_mat)
    return ex
end

# Brent 방법을 사용하여 근을 찾기
result = find_zero(GE_obj, (0.130, 0.160), Roots.Brent(), xtol=0.0001, ftol=0.0001, maxevals=20)

# 결과 출력
println("균형 rent: ", result)

end_str = Dates.format(now(), "yyyy-mm-dd HH:MM:SS")
println("종료시각: ", end_str)