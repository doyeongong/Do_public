PROGRAM Partial_retire
    ...
    REAL(4), PARAMETER :: phi = 0.3000d0           ! landlord fixed cost
    ...
    REAL :: share_homeowners=0, share_renters=0, share_landlords=0, share_owner_occupiers=0
    REAL :: share_h4=0, share_s4=0, share_trading=0, share_mortgage=0
  
    ! bisection-grid
    REAL(4) :: rho_lower= 0.160436347d0, rho, q_lower=1.49049056d0, q
    ...
    q_vec(1) = q_lower
    do j=2,rho_grid
      rho_vec(j) = rho_vec(j-1) + 0.00010d0
    enddo
    do i=2,q_grid
      q_vec(i) = q_vec(i-1) + 0.0010d0
    enddo
  ...
  ! 시뮬레이션 결과 출력
  OPEN(UNIT=1, FILE='W.txt', STATUS='UNKNOWN')
  OPEN(UNIT=2, FILE='H.txt', STATUS='UNKNOWN')
  OPEN(UNIT=3, FILE='DM.txt', STATUS='UNKNOWN')
  OPEN(UNIT=4, FILE='S.txt', STATUS='UNKNOWN')
  DO i = 1, population
    WRITE(1, '(5000F8.4)') (w_agent(i, t), t=1,time)
    WRITE(2, '(5000F8.1)') (h_agent(i, t), t=1,time)
    WRITE(3, '(5000F8.1)') (dm_agent(i, t), t=1,time)
    WRITE(4, '(5000F8.2)') (s_agent(i, t), t=1,time)
  END DO
  CLOSE(UNIT=1)
  CLOSE(UNIT=2)
  CLOSE(UNIT=3)
  ...
  END PROGRAM Partial_retire
    ...
    subroutine tax_induction(w, y, h_prime, s, mortgage, mortgage_rate, landlord_dummy, &
      ...
      ! 5억(11.83)이하 모기지 상환액 공제
      if (h_prime .le. 7 ) then
