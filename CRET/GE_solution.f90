program GE_solution  
    ...
    ! dimension of a solution
    integer, parameter :: n=2
    real, dimension(n) :: initialGuess

    ! Coefficients for NM algorithm
    real, parameter :: alpha = 1.0d0      ! reflexion
    real, parameter :: gamma = 2.0d0      ! expansion
    real, parameter :: rho = 0.5d0        ! contraction
    real, parameter :: sigma = 0.5d0      ! shrink

    ! define variables
    real, dimension(n+1, n) :: simplex, simplex_guess
    real, dimension(n+1) :: fValues, fValues_guess
    real, dimension(n) :: x_0, x_worst, x_r, x_e, x_c
    real :: f_c, f_e, f_r
  
    ! start time
    call CPU_TIME(start_time)

    print * , "-- A NM algorithm for finding a stationary equilibrium has started! --"

    ! Initialize simplex
    initialGuess(1) = 1.49049056d0
    initialGuess(2) = 0.160436347d0
    simplex(1,:) = initialGuess

    do i = 1, n
      simplex(i+1, :) = initialGuess
      simplex(i+1, i) = simplex(i+1, i) - 0.010d0
    end do

    call ordering(simplex, fValues)
    ...
    do iter = 1, maxIter
      ...
      ! Step 1: Ordering
      call order_simple(simplex, fValues)
      ! Step 2: Calculated the centroid
      x_0 = sum(simplex(1:n,:), dim=1) / real(n)

      ! Step 3: Reflection
      x_worst = simplex(n+1, :)
      x_r = x_0 + alpha * (x_0 - x_worst)
      call partial (x_r, f_r)
      
      if (fValues(1) <= f_r .and. f_r < fValues(n)) then
        ! <case1>
        simplex(n+1, :) = x_r
        fValues(n+1) = f_r
      elseif (f_r < fValues(1)) then
        ! <case2>
        x_e = x_0 + gamma * (x_r - x_0)
        call partial (x_e, f_e)
        if (f_e < f_r) then
          simplex(n+1,:) = x_e
          fValues(n+1) = f_e
        else
        ...
        ! <case3>
        if (fValues(n) <= f_r .and. f_r < fValues(n+1)) then
          x_c = x_0 + rho*(x_r - x_0)
          call partial(x_c, f_c)
          if (f_c < f_r) then
            simplex(n+1, :) = x_c
            fValues(n+1) = f_c
          else
    ...
    print *, "====Solution===="
    print *, "House price: ", simplex(1,1)
    print *, "Rent: ", simplex(1,2)
    print *, "Error for MCC: ", fValues(1)
  
  END PROGRAM GE_solution
    ...
    subroutine tax_induction(w, y, h_prime, s, mortgage, mortgage_rate, landlord_dummy, &
      ...
      real :: property_tax, tauM, deltaR, q, stand_ded, ytilde, income_tax, deduction
    ...
    subroutine partial(price, error_val)
      implicit none
      real, dimension(2) :: price
      real :: error_val, q, rho, H_d=0, S_d=0
      INTEGER :: i  
      ! Parameter
      ...
      REAL(4), PARAMETER :: tauC = 0.0039000d0         ! Comprehensive real estate tax
      REAL(4), PARAMETER :: phi = 0.30000d0           ! landlord fixed cost
      REAL(4), PARAMETER :: tauY_adj = 1.00000d0      ! tau_Y adjustment under Gov't BC holding case
      REAL(4), PARAMETER :: H_supply = 2.1950d0      ! House supply
      ...
      integer(4) :: counter=0, max_iter=1200, w, h, dm, h_prime, s, dm_prime, zeroequityind
      ...
      real :: consumption, Idummy, M, maxval_dm_prime
      ...
      REAL, DIMENSION(time) :: sum_wage=0.0d0, sum_h=0.0d0, sum_s=0.0d0
      ...
      call RANDOM_NUMBER(random_numbers)
        ...
        print *, "--------------------------------------"
        write(*,*) "House price :", q, "Rent :", rho
        write(*,*) "H demand :", H_d, "S demand :", S_d

        error_val = abs(H_d-H_supply) + abs(S_d-H_supply)

        write(*,*) "error of Market clear condition :", error_val
    ...
    subroutine ordering(simplex, fValues)
        implicit none
        real, dimension(3,2), intent(inout) :: simplex
        real :: f_val, temp_value
        real, dimension(2) :: temp_array
        real, dimension(3), intent(out) :: fValues
        real, dimension(2) :: price
        integer :: i, j, order
        ...
        do i = 1, 3
            price = simplex(i, 1:2)
            call partial (price, f_val)
            fValues(i) = f_val
        end do

        ! Sort the simplex and fValues
        do i = 1, 2
          order = i
          do j = i+1, 3
              if (fValues(j) < fValues(order)) then
                  order = j
              end if
          end do
          ! Swap values
          temp_value = fValues(i)
          fValues(i) = fValues(order)
          fValues(order) = temp_value
          ! Swap array rows
          temp_array = simplex(i, :)
          simplex(i, :) = simplex(order, :)
          simplex(order, :) = temp_array
      end do

     end

    subroutine order_simple(simplex, fValues)
      implicit none
      real, dimension(3,2), intent(inout) :: simplex
      real, dimension(3), intent(inout) :: fValues
      real :: object
      real, dimension(3,2) :: ordered_simplex
      real, dimension(3) :: ordered_fValues
      integer :: i, j, temp

      ordered_simplex = simplex
      ordered_fValues = fValues

      do i = 1, 3
          object = fValues(i)
          temp = 1
          do j = 1, 3
              if (object > fValues(j)) then
                  temp = temp + 1
              endif
          end do
          ordered_fValues(temp) = object
          ordered_simplex(temp, :) = simplex(i, :)
      end do

      simplex = ordered_simplex
      fValues = ordered_fValues
